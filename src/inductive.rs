...
impl<'t, 'p: 't> ExportFile<'p> {
    pub(crate) fn check_inductive_declar(&self, d: &Declar<'t>) {
        ...
        self.with_ctx(|ctx, arena| {
            ...
            let unmodified_tys_ctors = ctx.with_tc(env_limit, arena, |tc| {
            ...
            let mut st = ctx.with_tc(env_limit, arena, |tc| tc.specialize_nested(ind, unmodified_tys_ctors.clone()));

            // Check the (potentially modified) inductive specs against the base environment.
            ctx.with_tc(env_limit, arena, |tc| tc.check_inductive_specs(&mut st));
            ...
            ctx.with_tc_and_env_ext(&ind_ty_ext1, env_limit, arena, |tc| {
            ...
            let recursors = ctx.with_tc_and_env_ext(&ctor_extension, env_limit, arena, |tc| {
            ...
            ctx.with_tc_and_env_ext(&recursor_extension, env_limit, arena, |tc| {
                if st.is_nested() {
